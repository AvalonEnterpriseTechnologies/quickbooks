...
class QBSyncBills(models.AbstractModel):
    _name = 'qb.sync.bills'
    _description = 'QuickBooks Bill Sync'

    # ---- Odoo → QBO ----

    def _odoo_bill_to_qb(self, move):
        """Map an Odoo vendor bill to a QBO Bill dict."""
        vendor_ref = None
        ...
        for line in move.invoice_line_ids.filtered(lambda l: not l.display_type):
            ...
            # Use item-based if product has a QB Item ID
    ...
    def _qb_bill_to_odoo(self, qb_data, config):
        """Map a QBO Bill dict to Odoo account.move vals."""
        vals = {
            'move_type': 'in_invoice',
            'qb_bill_id': str(qb_data.get('Id', '')),
    ...
    def _apply_tax_ref(self, detail, line_vals, config):
        """Apply tax reference from QBO detail to Odoo line vals."""
    ...
    def push(self, client, config, job):
        ...
        if not qb_id:
            entity_data = matcher.find_qbo_match(client, 'bill', move)
            if entity_data:
                qb_id = str(entity_data.get('Id', ''))
                matcher.link_odoo_record(move, 'bill', entity_data)

        if qb_id:
            existing = client.read('Bill', qb_id)
            entity_data = existing.get('Bill', {})
            payload['Id'] = qb_id
            payload['SyncToken'] = entity_data.get('SyncToken', '0')
            payload['sparse'] = True
            resp = client.update('Bill', payload)
        else:
            resp = client.create('Bill', payload)

        created = resp.get('Bill', {})
        move.with_context(skip_qb_sync=True).write({
            'qb_bill_id': str(created.get('Id', '')),
    ...
    def pull(self, client, config, job):
        if job.qb_entity_id:
            resp = client.read('Bill', job.qb_entity_id)
            qb_data = resp.get('Bill', {})
        elif job.odoo_record_id:
            move = self.env['account.move'].browse(job.odoo_record_id)
            if not move.qb_bill_id:
                return {}
            resp = client.read('Bill', move.qb_bill_id)
            qb_data = resp.get('Bill', {})
        else:
            return {}
        ...
        vals = self._qb_bill_to_odoo(qb_data, config)
        ...
        if existing:
            ...
            decision = resolver.resolve(config, existing, qb_data, 'bill')
            if decision == 'qbo':
